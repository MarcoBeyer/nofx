                  ...
                  {(currentExchangeType === 'binance' ||
                    ...
                    currentExchangeType === 'alpaca') && (
                      <>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium" style={{ color: '#EAECEF' }}>
                                  <strong>{language === 'zh' ? '币安用户必读：' : 'Binance Users: '}</strong>
                                  {language === 'zh' ? '使用「现货与合约交易」API' : 'Use "Spot & Futures Trading" API'}
                                </span>
                              </div>
                        ...
                        {currentExchangeType === 'alpaca' && (
                          <div className="p-4 rounded" style={{ background: 'rgba(34, 197, 94, 0.1)', border: '1px solid rgba(34, 197, 94, 0.3)' }}>
                            <div className="text-sm font-semibold mb-2" style={{ color: '#22c55e' }}>{language === 'zh' ? '交易模式' : 'Trading Mode'}</div>
                            <label className="flex items-center gap-3 cursor-pointer">
                              <input type="checkbox" checked={testnet} onChange={(e) => setTestnet(e.target.checked)} className="w-5 h-5 rounded accent-green-500" />
                              <div>
                                <span className="text-sm font-medium" style={{ color: '#EAECEF' }}>{language === 'zh' ? '模拟交易 (Paper Trading)' : 'Paper Trading'}</span>
                                <div className="text-xs" style={{ color: '#848E9C' }}>{language === 'zh' ? '开启后使用 Alpaca 模拟账户' : 'Enable to use Alpaca paper trading account'}</div>
                              </div>
                            </label>
                          </div>
                        )}
