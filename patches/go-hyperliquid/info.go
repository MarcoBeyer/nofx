...
func NewInfo(
	...
	for _, spotInfo := range spotMeta.Universe {
		...
		// Bounds check: skip spot assets whose token index exceeds the Tokens slice length.
		// This can happen when Hyperliquid adds new spot tokens before updating the metadata.
		tokenIdx := spotInfo.Tokens[0]
		if tokenIdx < len(spotMeta.Tokens) {
			info.assetToDecimal[asset] = spotMeta.Tokens[tokenIdx].SzDecimals
		}
	}
